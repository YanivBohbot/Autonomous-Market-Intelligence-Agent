...
from app.agent.prompts.specialist_agent_prompts import CRM_SYSTEM_PROMPT
from app.agent.tools import crm_tool
from app.agent.graph import approval_node, route_after_approval
from app.agent.nodes.tool_utils import make_tool_runner

_CRM_TOOLS = [crm_tool]
_llm_with_tools = ChatOpenAI(model=settings.OPENAI_MODEL, temperature=0, streaming=True).bind_tools(_CRM_TOOLS)


def crm_agent_node(state: SupervisorState) -> Command:
    # See finance_agent.py for why this returns bare `Command` (no Literal
    # generic) instead of Command[Literal["approval", "supervisor"]].
    response = _llm_with_tools.invoke([SystemMessage(content=CRM_SYSTEM_PROMPT), *state["messages"]])
...
workflow.add_node("agent", crm_agent_node)
workflow.add_node("approval", approval_node)
workflow.add_node("tools", make_tool_runner(_CRM_TOOLS))
