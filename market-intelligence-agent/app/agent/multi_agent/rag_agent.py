...
_RAG_TOOLS = [search_knowledge_base_tool, web_search_tool]
_llm_with_tools = ChatOpenAI(model=settings.OPENAI_MODEL, temperature=0, streaming=True).bind_tools(_RAG_TOOLS)
...
workflow.add_node("agent", rag_agent_node)
workflow.add_node("approval", approval_node)
workflow.add_node("tools", make_tool_runner(_RAG_TOOLS))
