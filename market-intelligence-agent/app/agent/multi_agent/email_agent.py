...
_EMAIL_TOOLS = [send_email_tool]
_llm_with_tools = ChatOpenAI(model=settings.OPENAI_MODEL, temperature=0, streaming=True).bind_tools(_EMAIL_TOOLS)


def email_agent_node(state: SupervisorState) -> Command:
    # See finance_agent.py for why this returns bare `Command` (no Literal
    # generic) instead of Command[Literal["approval", "supervisor"]].
    response = _llm_with_tools.invoke([SystemMessage(content=EMAIL_SYSTEM_PROMPT), *state["messages"]])
...
workflow.add_node("tools", make_tool_runner(_EMAIL_TOOLS))
