...
from app.agent.tools import fs_read_file_tool, fs_list_dir_tool, fs_write_file_tool
from app.agent.graph import approval_node, route_after_approval
from app.agent.nodes.tool_utils import make_tool_runner

_FILESYSTEM_TOOLS = [fs_read_file_tool, fs_list_dir_tool, fs_write_file_tool]
_llm_with_tools = ChatOpenAI(model=settings.OPENAI_MODEL, temperature=0, streaming=True).bind_tools(_FILESYSTEM_TOOLS)


def filesystem_agent_node(state: SupervisorState) -> Command:
    # See finance_agent.py for why this returns bare `Command` (no Literal
    # generic) instead of Command[Literal["approval", "supervisor"]].
    response = _llm_with_tools.invoke([SystemMessage(content=FILESYSTEM_SYSTEM_PROMPT), *state["messages"]])
...
workflow.add_node("tools", make_tool_runner(_FILESYSTEM_TOOLS))
