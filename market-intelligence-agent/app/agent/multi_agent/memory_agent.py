...
from app.agent.tools import save_memory_tool, recall_memory_tool, list_memories_tool
from app.agent.graph import approval_node, route_after_approval
from app.agent.nodes.tool_utils import make_tool_runner

_MEMORY_TOOLS = [save_memory_tool, recall_memory_tool, list_memories_tool]
_llm_with_tools = ChatOpenAI(model=settings.OPENAI_MODEL, temperature=0, streaming=True).bind_tools(_MEMORY_TOOLS)


def memory_agent_node(state: SupervisorState) -> Command:
    # See finance_agent.py for why this returns bare `Command` (no Literal
    # generic) instead of Command[Literal["approval", "supervisor"]].
    response = _llm_with_tools.invoke([SystemMessage(content=MEMORY_SYSTEM_PROMPT), *state["messages"]])
...
workflow.add_node("tools", make_tool_runner(_MEMORY_TOOLS))
