...
| 21 | `concentration_screen` | read-only | Native (`finance_calc.py`) | portfolios (list of `{label, positions}`), threshold_pct (default 30), exclude_sectors (default `["ETF"]`) | Screens several labeled portfolios at once and returns exactly which portfolio+ticker pairs exceed `threshold_pct` of that portfolio's market value (`breaches`), computed via `compute_portfolio_metrics` per portfolio. Positions whose sector is in `exclude_sectors` still count toward totals/weights but are never listed in `breaches`. Replaces per-client `portfolio_metrics` calls for screening questions. | "Which clients have more than X% in a single stock" answers were being assembled by the LLM re-reading several `portfolio_metrics` results, which intermittently dropped a qualifying client even though the underlying data was correct. This tool makes the qualifying list itself deterministic (code-computed), not LLM-synthesized prose. `exclude_sectors` defaults to excluding ETFs, which otherwise produced false-positive breaches (a diversified bond/index fund dominating a portfolio by design is not a single-stock concentration risk). |
...
- **File:** `app/agent/tools/finance_calc.py`
- **What:** Given several labeled portfolios (`{label, positions}`, same `Position` shape as `portfolio_metrics`), a `threshold_pct` (default 30), and `exclude_sectors` (default `["ETF"]`), computes `portfolio_metrics` for each portfolio and returns `breaches`: every `{label, ticker, weight_pct, market_value, portfolio_market_value}` whose `weight_pct` exceeds the threshold AND whose sector is not in `exclude_sectors`, sorted by weight descending. Excluded-sector positions still count toward that portfolio's totals/weights — only their appearance in `breaches` is suppressed. Also returns `screened_count`/`screened_labels` for audit. Rejects an empty portfolio list, a non-positive threshold, or duplicate portfolio labels. Read-only; in `READ_ONLY_TOOLS`. Replaces making a separate `portfolio_metrics` call per client for screening questions ("which clients are concentrated / over X%").
- **Why:** Live grounded QA found that "which clients have more than X% in a single stock" answers, built by the LLM re-reading several separate `portfolio_metrics` tool results, intermittently dropped a qualifying client from the final prose even though every tool result had the correct `weight_pct` — a real correctness risk for an advisor-facing tool. This tool moves the "who qualifies" decision into deterministic code so the LLM only relays `breaches`, eliminating that failure mode. `exclude_sectors` defaults to `["ETF"]` because a diversified bond/index ETF legitimately dominating a conservative client's portfolio by design is not a single-stock concentration risk; without the exclusion it produced false-positive breaches that buried genuine single-stock overweights.
