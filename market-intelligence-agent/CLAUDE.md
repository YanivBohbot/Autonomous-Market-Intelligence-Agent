...
| `concentration_screen` | `app/agent/tools/finance_calc.py` | read-only | Screens several labeled portfolios (`{label, positions}`) at once against a `threshold_pct` (default 30) and returns exactly which portfolio+ticker pairs breach it (`breaches`), computed via `portfolio_metrics` per portfolio; `exclude_sectors` (default `["ETF"]`) keeps those positions in totals/weights but out of `breaches` — keeps "which clients are concentrated in a single stock" answers deterministic/code-computed instead of LLM-synthesized prose, and replaces per-client `portfolio_metrics` calls for screening questions. |
